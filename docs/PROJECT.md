...
한 번의 새 분석에는 공고 조사, 기업 조사, 보고서 생성의 AI 호출이 순서대로 실행된다. 각 단계는 일시적인 API 오류나 결과 검증 실패가 발생하면 총 3회까지 다시 생성한다. 공고·기업 조사 호출에는 이력서, 개인 선호, 연락처를 보내지 않는다. 마지막 보고서 생성 호출에서만 선택적인 사용자 경험을 사용한다.
...
서버는 기업 조사와 보고서 생성의 두 OpenAI 호출을 순서대로 실행한다. 각 단계는 일시적인 API 오류, 요청 제한, 미완료 응답, JSON·스키마·근거 검증 실패가 발생하면 같은 단계만 총 3회까지 다시 시도한다. 설정·권한 오류, AI 거절, 잘못된 입력, 사용자 취소와 시간 초과는 재시도하지 않는다. 법인이 모호하고 후보가 있으면 `{ "type": "needs_company", "candidates": [...] }`를 반환해 선택 또는 매출 생략을 받는다. 그 밖에는 검사를 통과한 `{ "report": Report }` 단일 JSON 응답을 반환한다. 현재 NDJSON 스트리밍이나 부분 보고서는 구현하지 않았다. 전체 작업은 110초에 중단되며 라우트의 Vercel 최대 실행 시간은 120초다.
...
- 공고 조사까지 포함한 새 분석은 정상적으로 3회 호출하며, 세 단계가 모두 최대 횟수까지 실패하면 최대 9회 호출될 수 있다. 실제 정확도·지연·비용은 모델, 검색 결과, 계정 한도에 따라 달라진다.
